...
import { getLatestTianhePosition, useTianheHook } from '../../../Api/tianheApi'
...
function formatUpdatedLabel(dataUpdatedAt: number | undefined) {
    if (dataUpdatedAt == null) return '—'
    return new Date(dataUpdatedAt).toLocaleString()
}

type IssMapInnerProps = {
    title: string
    lat: number
    lon: number
    updatedLabel: string
...
function IssMapIss() {
    ...
    const lat = Number(issQuery.data?.iss_position.latitude ?? 0)
    const lon = Number(issQuery.data?.iss_position.longitude ?? 0)
    return (
        <IssMapInner
            title="ISS"
            lat={lat}
            lon={lon}
            updatedLabel={formatUpdatedLabel(issQuery.dataUpdatedAt)}
            markerAriaLabel="International Space Station position"
        />
    )
}
...
type IssMapProps = { variant: 'iss' | 'tianhe' }

/** Each variant runs its own station query and shows last-updated next to the map controls. */
export const IssMap = ({ variant }: IssMapProps) => {
    if (variant === 'iss') return <IssMapIss />
    return <IssMapTianhe />
}
