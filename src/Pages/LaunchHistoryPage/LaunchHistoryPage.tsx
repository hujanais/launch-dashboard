...
export const LaunchHistoryPage = () => {
    ...
    const latestCompletedLaunch = useMemo(() => {
        const completed = launches.results.filter(
            (launch) => isSuccessfulLaunch(launch) || isFailedLaunch(launch)
        )
        if (completed.length === 0) return null

        return [...completed].sort((a, b) => {
            const bTime = Date.parse(b.net || b.window_end || b.last_updated)
            const aTime = Date.parse(a.net || a.window_end || a.last_updated)
            return bTime - aTime
        })[0]
    }, [launches.results])

    const latestCompletedLaunchStatus = useMemo(() => {
        if (!latestCompletedLaunch) return null
        if (isSuccessfulLaunch(latestCompletedLaunch)) return 'Success'
        if (isFailedLaunch(latestCompletedLaunch)) return 'Fail'
        return null
    }, [latestCompletedLaunch])
        ...
        <div className={styles.scrollableContainer}>
            <header className={styles.topBar}>
                ...
                <div className={styles.metaRow}>
                    <Typography
                        variant="body2"
                        className={`${styles.metaLine} ${styles.metaInline}`}
                    >
                        Total launches: {launchTotals.launches} | Successes:{' '}
                        {launchTotals.success} | Failures: {launchTotals.failures}
                    </Typography>
                    <Typography
                        variant="body2"
                        className={`${styles.metaLine} ${styles.metaInline}`}
                    >
                        Latest launch:{' '}
                        {latestCompletedLaunch
                            ? `${latestCompletedLaunch.name} (${new Date(
                                  latestCompletedLaunch.net ||
                                      latestCompletedLaunch.window_end ||
                                      latestCompletedLaunch.last_updated
                              ).toLocaleString()})${
                                  latestCompletedLaunchStatus
                                      ? ` [${latestCompletedLaunchStatus}]`
                                      : ' [status unavailable]'
                              }`
                    ...
                    <Typography
                        variant="caption"
                        className={`${styles.metaLine} ${styles.metaTimestamp}`}
                    >
                        Last updated:{' '}
                        {lastUpdated
